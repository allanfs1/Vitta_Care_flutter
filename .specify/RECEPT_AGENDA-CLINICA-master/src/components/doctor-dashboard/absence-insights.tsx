...
interface AbsenceInsightsProps {
...
export function AbsenceInsights({ absenceData }: AbsenceInsightsProps) {

  return (
    <div className="w-full">
      {absenceData && absenceData.length > 0 ? (
        <div className="space-y-3">
          {absenceData.map((absence) => <AbsenceItem key={absence.id} absence={absence} />)}
        </div>
      ) : (
