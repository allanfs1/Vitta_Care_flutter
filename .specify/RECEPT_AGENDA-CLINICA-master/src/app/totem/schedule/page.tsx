...
import { Check, ChevronsUpDown, Clock, Heart, ArrowLeft, CalendarIcon, Timer, User } from 'lucide-react';
...
export default function ScheduleAppointmentPage() {
  ...
  const [allDoctorsForDate, setAllDoctorsForDate] = React.useState<Doctor[]>([]);
  const [filterDoctorId, setFilterDoctorId] = React.useState<string | null>(null);

  // Define um ID de clínica estático para o totem. Em uma aplicação real, isso viria de uma configuração.
  const CLINIC_ID = "3v4aHjYCLdC0vHSozB9b";
  ...
  React.useEffect(() => {
    const fetchAvailableSlots = async () => {
      ...
      try {
        ...
        // Constrói a query inicial com o filtro da clínica
        let queries = [where("idclinica", "==", CLINIC_ID)];

        if (selectedSpecialty !== 'Todos') {
            queries.push(where('especialidades', 'array-contains', selectedSpecialty));
        }

        const medicosQuery = query(medicosRef, ...queries, limit(30));
        
        const medicosSnapshot = await getDocs(medicosQuery);

        if (medicosSnapshot.empty) {
          console.log(`Nenhum médico encontrado para a especialidade: ${selectedSpecialty} na clínica ${CLINIC_ID}`);
          setAvailableSlots([]);
          setLoading(false);
          return;
        }
        ...
        const doctorsWithSlotsThisDay: Doctor[] = [];
            ...
            if (!hourScheduleSnapshot.empty) {
                ...
                if (scheduleDoc.statusSM && scheduleDoc.statusSM[dayOfWeek]) {
                     ...
                     if (startHour && endHour) {
                         doctorsWithSlotsThisDay.push(doctor);
        ...
        // Create a unique list of doctors for the filter buttons
        const uniqueDoctors = doctorsWithSlotsThisDay.filter((doc, index, self) =>
            index === self.findIndex((d) => d.id === doc.id)
        );
        setAllDoctorsForDate(uniqueDoctors);
  ...
  const handleTimeSelection = (slot: AvailableSlot) => {
    ...
    setFilterDoctorId(slot.doctor.id); // Also set filter when a slot is directly clicked
  }
  ...
  const toggleDoctorFilter = (doctorId: string) => {
    if (filterDoctorId === doctorId) {
        setFilterDoctorId(null); // Unselect if clicked again
    } else {
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-muted/40 p-4">
      <Card className="w-full max-w-md shadow-2xl">
        ...
        <CardContent className="space-y-8">
          ...
          {/* Doctor Filter */}
          {!loading && allDoctorsForDate.length > 0 && (
            <div className="space-y-2">
                <label className="text-sm font-medium">Filtrar por Médico</label>
                <div className="flex flex-wrap gap-2">
                     <Button
                        variant={filterDoctorId === null ? 'default' : 'outline'}
                        size="sm"
                    ...
                    {allDoctorsForDate.map(doctor => (
                        <Button
                            key={doctor.id}
                            variant={filterDoctorId === doctor.id ? 'default' : 'outline'}
                            size="sm"
                            onClick={() => toggleDoctorFilter(doctor.id)}
                            className="rounded-full"
                        >
                            <User className="mr-2 h-4 w-4" />
                            {doctor.nomeCompleto.split(' ')[0]}
                        </Button>
                    ))}
                </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
                ...
                {availableSlots.map((slot) => {
                    const isSelected = selectedTime === slot.time && selectedDoctor?.id === slot.doctor.id;
                    const isDoctorFiltered = filterDoctorId === slot.doctor.id;
                    const isDimmed = filterDoctorId !== null && !isDoctorFiltered;
                    ...
                    return (
                        <Button
                            ...
                            className={cn(
                                'h-12 text-base relative transition-all',
                                isDoctorFiltered && 'bg-primary/20 border-primary',
                                isDimmed && 'opacity-40 hover:opacity-60',
                                slot.isPreBooked && !isDimmed && 'border-yellow-500 text-yellow-600',
                                isSelected && slot.isPreBooked && 'bg-yellow-500/80 text-white'
                            )}
                        >
                            {slot.isPreBooked && (
          ...
          <div className="flex flex-col gap-4">
              <Button size="lg" className="w-full h-14 text-lg" disabled={!selectedTime || loading} onClick={handleSchedule}>
                Agendar
              </Button>
