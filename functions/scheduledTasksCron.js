...
async function executeTask(task) {
  ...
  let reportId = null;
  if (isReport) {
    // Grava em `tb_relatorio_ia` — a coleção que a tela /relatorios lê. Antes
    // ia só para `tb_scheduled_reports`, então um relatório produzido por uma
    // rotina aprovada não aparecia para quem aprovou a rotina.
    const base = {
      titulo: task.titulo, markdown: content, conteudo: content,
      periodo: task.scheduleLabel || "Recorrente",
      taskId: task.id, idclinica: clinicaId,
      origem: "cron", createdAt: FieldValue.serverTimestamp(),
    };
    const ref = await db.collection("tb_relatorio_ia")
      .add({ ...base, tipoRelatorio: "ia" });
    // Trilha por tarefa, que alimenta o histórico de execuções.
    await db.collection("tb_scheduled_reports").doc(ref.id)
      .set({ ...base, tipoRelatorio: "agendado", relatorioId: ref.id });
    reportId = ref.id;
...
// ─────────────────────── Reuso pelo Vigia ───────────────────────
//
// `vigiaCron.js` monta seu próprio prompt mas roda sobre o MESMO loop de
// ferramentas e o mesmo circuit breaker de leituras. Exportar aqui evita uma
// segunda implementação do agente em Node — que divergiria da primeira na
// primeira correção que alguém fizesse só de um lado.
exports._runAgent = runAgent;
exports._db = () => db;
exports._makeReadMeter = () => makeReadMeter(READ_BUDGET);
